 ...
 * SemiFuture. Callers MUST use the timeout parameter or abort() to ensure
 * bounded completion — without either, operations may wait indefinitely
 * for IB completion.
 *
 ...
 *     NOTE: timeout is currently only implemented for write().
 *
 *   commInternalError — IB / transport-level failure:
 *     write(), read(), waitForWrite()
 *
 *   commUserAbort — abort() was called while the operation was pending:
 *     write(), read(), waitForWrite()
 *
